...
namespace Geodesic
{
	...
	bool CICH_WindowFiltering::CheckValidityWithXinWangFiltering(Window& w) const
	{
		...
		double detaX = w.coordOfPseudoSource(0) - w.proportions[1] * edge.length;
		double rightLen = sqrt(detaX * detaX + w.coordOfPseudoSource(1) * w.coordOfPseudoSource(1));
		...
		detaX = w.coordOfPseudoSource(0) - w.proportions[0] * edge.length;
		double leftLen = sqrt(detaX * detaX + w.coordOfPseudoSource(1) * w.coordOfPseudoSource(1));
		...
		double xOfVert = edge.length - oppositeEdge.coordOfOppositeVert(0);
